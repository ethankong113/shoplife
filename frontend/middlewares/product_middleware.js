import { CREATE_PRODUCT, READ_PRODUCT, UPDATE_PRODUCT, DELETE_PRODUCT, receiveProduct, receiveErrors } from '../actions/product_actions';
...
export const ProductMiddleware = ({getState, dispatch}) => next => action => {
  const appendProductCB = product => {dispatch(appendProduct(product));};
  const receiveProductCB = product => {dispatch(receiveProduct(product));};
  const renewProductCB = product => {dispatch(renewProduct(product));};
